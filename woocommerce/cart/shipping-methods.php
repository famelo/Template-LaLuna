...
if ( $available_methods ) {
	...
	foreach ( $available_methods as $method ) {
		$method->full_label = '<span class="name">' . $method->label . '</span>';

		if ( $method->cost > 0 ) {
			if ( $woocommerce->cart->tax_display_cart == 'excl' ) {
				$method->full_label .= '<span class="delimiter">: </span>' . woocommerce_price( $method->cost );
				...
				$method->full_label .= '<span class="delimiter">: </span>' . woocommerce_price( $method->cost + $method->get_shipping_tax() );
				if ( $method->get_shipping_tax() > 0 && ! $woocommerce->cart->prices_include_tax ) {
