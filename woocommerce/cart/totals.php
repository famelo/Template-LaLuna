	...
	<?php if ( ! $woocommerce->shipping->enabled || $available_methods || ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->has_calculated_shipping() ) : ?>
		...
		<table cellspacing="0">
			<tbody>
				...
				<?php
					if( get_option( WGM_Helper::get_wgm_option( 'woocommerce_de_kleinunternehmerregelung' ) ) == 'off' ) {
					...
					<td>
						...
						<?php
							...
							if (  $woocommerce->cart->tax_display_cart == 'incl' ) {
								...
								if ( ! empty( $tax_string_array ) && get_option( WGM_Helper::get_wgm_option( 'woocommerce_de_kleinunternehmerregelung' ) ) == 'off' ) {
									echo '<small class="includes_tax">' . sprintf( __( '(Includes %s)', Woocommerce_German_Market::get_textdomain() ), implode( ', ', $tax_string_array ) ) . '</small>';
								}
							}
						?>
					</td>
		...
		<?php if ( $woocommerce->cart->get_cart_tax() && ( get_option( 'woocommerce_de_estimate_cart' ) == 'on' ) ) : ?>
